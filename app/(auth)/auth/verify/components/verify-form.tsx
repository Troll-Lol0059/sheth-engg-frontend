...
const VerifyForm = ({ email, type }: VerifyFormProps) => {
	...
	const onResend = async () => {
		startCounter(59);
		try {
			await axios.post("/api/v1/user/resend-otp", { email });
	...
	const onVerify = async (values: z.infer<typeof VerificationSchema>) => {
		...
		const { otp } = values;
		const response = await axios.post("/api/v1/user/verify-reset-otp", { email, otp });
	...
	const { mutate, isPending } = useMutation({
		...
		onSuccess: (data: Record<string, unknown>) => {
			if (type === "LOGIN") {
				...
				const resetToken = (data?.data as Record<string, unknown>)?.resetToken;
				toast.success("Success!", { id: onVerifyToast, description: "OTP verified successfully!" });
				router.replace(`/auth/reset-password?email=${encodeURIComponent(email)}&token=${resetToken}`);
			} else {
				router.replace("/auth/login");
			}
		},
	...
	return (
		<Form {...form}>
			<form className="flex w-full max-w-md flex-col items-center justify-stretch gap-3 px-4 sm:gap-4 md:gap-6 md:px-0" onSubmit={form.handleSubmit(values => mutate(values))}>
				<FormField
					...
					render={({ field }) => (
						<FormItem className="w-full">
							<FormControl>
								<InputOTP className="flex w-full justify-center" containerClassName="gap-1 sm:gap-2 md:gap-3" maxLength={6} {...field}>
									{Array.from({ length: 6 }, (_, index) => (
										<InputOTPGroup key={`otp-slot-${index}`}>
											<InputOTPSlot index={index} className="h-8 w-8 text-xs sm:h-10 sm:w-10 md:h-14 md:w-14 md:text-base" />
										</InputOTPGroup>
									))}
								</InputOTP>
							</FormControl>
				...
				<div className="flex w-full flex-col items-center justify-between gap-3 md:gap-4">
					<Button className="border-rounded h-8 w-1/2 cursor-pointer rounded-full font-medium" disabled={isPending} size="lg" type="submit" variant="default">
					...
					{counter > 0 ? (
						...
						<Button className="w-full max-w-[180px] cursor-pointer gap-2" disabled={isPending} onClick={onResend} size="lg" type="button" variant="ghost">
							<RefreshCcw className="h-4 w-4 sm:h-5 sm:w-5" />
							Resend OTP
						</Button>
					)}
					<Button className="w-36 cursor-pointer" disabled={isPending} onClick={router.back} size="sm" type="button" variant="ghost">
						<ArrowLeft />
						Back
					</Button>
				</div>
