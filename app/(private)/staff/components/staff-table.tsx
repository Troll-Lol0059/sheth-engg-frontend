...
const StaffTable = ({ initialData, columns, totalPages, totalElements }: StaffTableProps) => {
	...
	const fetchStaff = async ({ queryKey }: { queryKey: QueryKey }) => {
		...
		try {
			...
			const response = await axios.get(`/api/v1/staff?${params.toString()}`);
	...
	const { data, isFetching } = useQuery<Staff[]>({
		queryKey: ["staff-all", globalFilter, pagination.pageIndex, pagination.pageSize, sorting[0]?.id, sorting[0]?.desc ? "desc" : "asc"],
		queryFn: fetchStaff,
	...
	return (
		<>
			<div className="flex w-full flex-col items-center justify-end gap-1 py-3 sm:flex-row">
				<Input className="h-10 max-w-sm" onChange={e => setFilter(e.target.value)} placeholder="Search staff..." startContent={<Search size={16} />} value={filter} />
			...
			<TablePaginationControls footerHeading="Staff" table={table} totalElementsCount={totalElementsCount} />
		</>
	);
};
