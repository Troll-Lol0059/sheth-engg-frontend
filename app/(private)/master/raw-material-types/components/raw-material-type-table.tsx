...
type RawMaterialTypeTableProps = {
	initialData: RawMaterialType[];
	totalPages: number;
	columns: ColumnDef<RawMaterialType>[];
	totalElements: number;
};

const RawMaterialTypeTable = ({ initialData, columns, totalPages, totalElements }: RawMaterialTypeTableProps) => {
	...
	const fetchRawMaterialTypes = async ({ queryKey }: { queryKey: QueryKey }) => {
		...
		try {
			...
			const response = await axios.get(`/api/v1/master/raw-material-type/all?${params.toString()}`);
			const result = response?.data?.data;
			setPageCount(result?.totalPages ?? 0);
			setTotalElementsCount(result?.totalCount ?? 0);
			return (result?.data ?? []) as RawMaterialType[];
		} catch (error: unknown) {
	...
	const { data, isFetching } = useQuery<RawMaterialType[]>({
		queryKey: [
			"raw-material-types-all",
			globalFilter,
	...
	return (
		<>
			<div className="flex w-full flex-col items-center justify-end gap-1 py-3 sm:flex-row">
				<Input className="h-10 max-w-sm" onChange={e => setFilter(e.target.value)} placeholder="Search raw material types..." startContent={<Search size={16} />} value={filter} />
			...
			<TablePaginationControls footerHeading="Raw Material Types" table={table} totalElementsCount={totalElementsCount} />
		</>
	);
};

export default RawMaterialTypeTable;
