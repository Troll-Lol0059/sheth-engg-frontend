...
type HardnessTypeTableProps = {
	initialData: HardnessType[];
	totalPages: number;
	columns: ColumnDef<HardnessType>[];
	totalElements: number;
};

const HardnessTypeTable = ({ initialData, columns, totalPages, totalElements }: HardnessTypeTableProps) => {
	...
	const fetchHardnessTypes = async ({ queryKey }: { queryKey: QueryKey }) => {
		...
		try {
			...
			const response = await axios.get(`/api/v1/master/hardness-type/all?${params.toString()}`);
			const result = response?.data?.data;
			setPageCount(result?.totalPages ?? 0);
			setTotalElementsCount(result?.totalCount ?? 0);
			return (result?.data ?? []) as HardnessType[];
		} catch (error: unknown) {
	...
	const { data, isFetching } = useQuery<HardnessType[]>({
		queryKey: [
			"hardness-types-all",
			globalFilter,
	...
	return (
		<>
			<div className="flex w-full flex-col items-center justify-end gap-1 py-3 sm:flex-row">
				<Input className="h-10 max-w-sm" onChange={e => setFilter(e.target.value)} placeholder="Search hardness types..." startContent={<Search size={16} />} value={filter} />
			...
			<TablePaginationControls footerHeading="Hardness Types" table={table} totalElementsCount={totalElementsCount} />
		</>
	);
};

export default HardnessTypeTable;
